...
EFI_STATUS
EFIAPI
MyUSBKeyboardDriverBindingEntryPoint (
  IN EFI_HANDLE        ImageHandle,
  IN EFI_SYSTEM_TABLE  *SystemTable
  )
{

  DEBUG((DEBUG_INFO, "MyUsbKbDxe: Entry point executed\n"));
...
{
  DEBUG((DEBUG_INFO, "USBKBDXE SUPPORTED\n"));
  EFI_STATUS           Status;
  ...
  if (!IsUSBKeyboard (UsbIo)) {
    DEBUG((DEBUG_INFO, "Supported SUCCESS FOR MYUSBKBDXE\n"));
...
{
  DEBUG((DEBUG_INFO, "MYUSBKBDXE STARTED\n"));
  EFI_STATUS                   Status;
  EFI_USB_IO_PROTOCOL          *UsbIo;
