  ...
  )
{
  ...
  /* --- Left-handed mode button handling --- */
  BOOLEAN Left;
  BOOLEAN Right;

  /* Read raw HID button bits */
  Left  = (BOOLEAN)((*(UINT8 *)Data & BIT0) != 0);
  Right = (BOOLEAN)((*(UINT8 *)Data & BIT1) != 0);

  #if LEFT_HANDED_MODE
    /* Swap left and right buttons */
    UsbMouseDevice->State.LeftButton  = Right;
    UsbMouseDevice->State.RightButton = Left;
  #else
    /* Normal button mapping */
    UsbMouseDevice->State.LeftButton  = Left;
    UsbMouseDevice->State.RightButton = Right;
  #endif

/* --- End of left-handed mode logic --- */
